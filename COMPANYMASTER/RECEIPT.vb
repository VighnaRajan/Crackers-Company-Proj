...
Public Class RECEIPT
    ...
    Private Sub RECEIPT_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        writer.Close()
        writer.Dispose()
    End Sub
    ...
    Private Sub suppliers_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles suppliers.SelectedIndexChanged
        ...
        opbaltype = supds.Tables(0).Rows(suppliers.SelectedIndex).Item("drcr")  'dr means they have to give money for us
        ...
        'find total credit sales made to selected customer
        t1cmd.CommandText = "select sum(amt) from trans where dr='" & scode & "' and typeoftrans='CREDIT SALES' and tdate>=#" & opbaldate & "#"
        t1ds.Reset()
        t1ds.Clear()
        t1da.SelectCommand = t1cmd
        t1da.Fill(t1ds)
        If IsDBNull(t1ds.Tables(0).Rows(0).Item(0)) Then totdr = 0 Else totdr = t1ds.Tables(0).Rows(0).Item(0)
        'find total cash receipts made from the selected customer
        t2cmd.CommandText = "select sum(amt) from trans where cr='" & scode & "' and dr='CASH' and typeoftrans='CASH RECEIPT' and tdate>=#" & opbaldate & "#"
        t2ds.Reset()
        t2ds.Clear()
        t2da.SelectCommand = t2cmd
        t2da.Fill(t2ds)
        If IsDBNull(t2ds.Tables(0).Rows(0).Item(0)) Then totcr = 0 Else totcr = t2ds.Tables(0).Rows(0).Item(0)
...
adddr:
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        nucmd.CommandText = "insert into trans values(" & ccode & ",#" & dop.Value.Date & "#,'CASH RECEIPT','CASH','" & scode & "'," & Val(amt.Text) & ",'CASH RECEIPT made as on " & dop.Value.Date & "')"
