...
Imports System.IO
Public Class createFINISHEDPRODUCT
    ...
    Private Sub createFINISHEDPRODUCT_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        writer.Close()
        writer.Dispose()
    End Sub
    Private Sub createFINISHEDPRODUCT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        con.ConnectionString = cs '"Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & cs
        con.Open()
        rawcmd.CommandText = "select * from items where ucase(itemtype)='PRODUCT' and concerncode=" & ccode
...
nextj:
    ...
    Private Sub addnew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles addnew.Click
        ...
        Else
            ...
            Next
            ff = MsgBox("Save New Product?", MsgBoxStyle.OkCancel, "CONFIRMATION")
            If ff = MsgBoxResult.Ok Then
                dupcmd.CommandText = "select * from items where ucase(itemcode)='" & UCase(itemcode.Text) & "' and concerncode=" & ccode & " and itemtype='PRODUCT'"
                dupds.Reset()
                dupds.Clear()
                ...
                If dupds.Tables(0).Rows.Count > 0 Then
                    ...
                    itemcode.Focus()
                Else
                    nucmd.CommandText = "insert into items values(" & ccode & ",'" & itemcode.Text & "','" & itemname.Text & "','PRODUCT','" & m(1) & "','" & m(2) & "')"
                    writer.WriteLine("Adding New Product ...")
                    writer.WriteLine("Query : " & nucmd.CommandText)
                    nucmd.ExecuteNonQuery()
                    raws.Items.Add(itemname.Text)
                    MsgBox("NEW PRODUCT ADDED", MsgBoxStyle.Information, "SUCCESS")
                    addnew.Text = "&Add New"
                    editt.Enabled = True
                    box_readonly(1)
                End If
    ...
    Private Sub editt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles editt.Click
        ...
        Else
            ...
            For j = 1 To measure.CheckedItems.Count
                m(j) = measure.CheckedItems(j - 1)
            Next
            ff = MsgBox("Save Corrections in Product?", MsgBoxStyle.OkCancel, "CONFIRMATION")
            If ff = MsgBoxResult.Ok Then
                nucmd.CommandText = "update items set itemname='" & itemname.Text & "',measure='" & m(1) & "',measure1='" & m(2) & "' where itemcode='" & itemcode.Text & "' and concerncode=" & ccode & " and itemtype='PRODUCT'"
                writer.WriteLine("Query : " & nucmd.CommandText)
                nucmd.ExecuteNonQuery()
                raws.Items(raws.SelectedIndex) = itemname.Text
                MsgBox("PRODUCT ENTRY CORRECTED", MsgBoxStyle.Information, "SUCCESS")
                editt.Text = "&Edit"
                addnew.Enabled = True
                box_readonly(0)
            Else
