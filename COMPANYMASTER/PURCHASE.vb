...
Public Class PURCHASE
    ...
    Private Sub PURCHASE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        icmd.CommandText = "select * from items where concerncode=" & ccode & " and itemtype<>'PRODUCT' and itemtype<>'WASTE'"
        ids.Reset()
        ids.Clear()
        ...
        suppcmd.CommandText = "select * from parties where concerncode=" & ccode & " and partytype='SUPPLIER'"
        suppds.Reset()
        suppds.Clear()
    ...
    Private Sub itms_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itms.SelectedIndexChanged
        ...
        If tds.Tables(0).Rows.Count > 0 Then 'already purchase entry exist on same date. ? to do
            ff = MsgBox("Already Purchase of the Item : " & itms.Text & " Exist on this same date. Add another Purchase?", MsgBoxStyle.YesNo, "PURCHASE ALREADY EXIST. ADD ANOTHER - CONFIRMATION")
            ...
            Else
                ...
                ''prohibited now
                'Else
                MsgBox("PURCHASE PROCESS CENCELLED BY USER", MsgBoxStyle.Critical, "USER ABORT")
                'End If
            End If
...
fresh_purchase:
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If suppliers.SelectedIndex < 0 Then
            MsgBox("Select any Supplier", MsgBoxStyle.Critical, "SUPPLIER CAN NOT BE LEFT BLANK")
            suppliers.Focus()
            GoTo out
        End If
        ff = MsgBox("Cash Purchase?", MsgBoxStyle.YesNo, "CASH PURCHASE? - CONFIRMATION")
        dr = ids.Tables(0).Rows(itms.SelectedIndex).Item("itemcode") 'item comes in
        If ff = MsgBoxResult.Yes Then
            tot = "CASH PURCHASE"
            cr = "CASH" 'cash goes out
            cmnt = "CASH PURCHASE as per invoice no." & bno.Text & " dated :" & dop.Value.Date
        Else
            tot = "CREDIT PURCHASE"
            cr = suppds.Tables(0).Rows(suppliers.SelectedIndex).Item("code")
            cmnt = "CREDIT PURCHASE as per invoice no." & bno.Text & " dated :" & dop.Value.Date
        End If
        'find closing stock
        ...
        writer.WriteLine("Adding PURCHASE TO ITEMSTRANS ...")
        ...
        End If
        writer.WriteLine("Adding PURCHASED QUANTITY To STOCK...")
        writer.WriteLine("Query : " & nucmd.CommandText)
        nucmd.ExecuteNonQuery()
        ...
        MsgBox("PURCHASE ENTRY MADE", MsgBoxStyle.Information, "SUCCESS")
        bno.Clear()
        q1.Clear()
        q2.Clear()
        rate.Clear()
